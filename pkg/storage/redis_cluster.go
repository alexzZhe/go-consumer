...
var connectionStatus = make(chan bool, 1)
...
func (r *RedisCluster) LPush(keyName string, values ...string) error {
	if err := r.up(); err != nil {
		return err
	}
	...
	err := cluster.LPush(r.fixKey(keyName), values).Err()
	if err != nil {
		log.Debugf("Error trying to LPush value: %s", err.Error())
...
func (r *RedisCluster) BRPop(keyName string, timeout int) ([]string, error) {
	if err := r.up(); err != nil {
		return nil, err
	}
	...
	value, err := cluster.BRPop(time.Duration(timeout)*time.Second, r.fixKey(keyName)).Result()
	if err != nil && !errors.Is(err, redis.Nil) {
		log.Debugf("Error trying to BRPop value: %s", err.Error())
	}
